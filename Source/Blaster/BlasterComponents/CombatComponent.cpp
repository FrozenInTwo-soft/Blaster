...
void UCombatComponent::SpawnDefaultWeapon()
{
	...
	if (BlasterGameMode && World && Character && !Character->IsElimmed() && DefaultWeaponClass)
	{
...
inline bool UCombatComponent::ServerFire_Validate(const FVector_NetQuantize& TraceHitTarget, float FireDelay)
{
	if (EquippedWeapon)
	{
...
{
	MulticaseShotgunFire(TraceHitTargets);
}

inline bool UCombatComponent::ServerShotgunFire_Validate(const TArray<FVector_NetQuantize>& TraceHitTargets,
	float FireDelay)
{
	if (EquippedWeapon)
	{
...
void UCombatComponent::MulticaseShotgunFire_Implementation(const TArray<FVector_NetQuantize>& TraceHitTargets)
{
	if (Character && Character->IsLocallyControlled() && !Character->HasAuthority()) return;
...
inline void UCombatComponent::AttachActorToBackpack(AActor* ActorToAttach)
{
	if (Character == nullptr || Character->GetMesh() == nullptr || ActorToAttach == nullptr) return;
...
inline void UCombatComponent::UpdateCarriedAmmo()
{
	if (EquippedWeapon == nullptr) return;
